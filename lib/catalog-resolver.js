...
export async function resolveCatalogCard({ imageDataUrl, aiCard, client, model }) {
  ...
  if (hasCompleteCollector) {
    // HARD RULE:
    // when a complete collector number is available, the catalog may NEVER replace it
    // with a different printing. This prevents e.g. Black Bolt Victini becoming Champion's Path.
    ...
    if (exactCollectorMatches.length === 1) {
      ...
      confidence = 0.995;
      method = "exact_name_collector";
    } else if (exactCollectorMatches.length > 1) {
      // Only use vision to choose BETWEEN cards with the SAME exact collector fraction.
      try {
        const choice = await visionChoose(client, model, imageDataUrl, exactCollectorMatches);
        if (choice && choice.confidence >= 0.88) {
    ...
    if (!selected) {
      const source = exactCollectorMatches.length ? exactCollectorMatches : ranked;
      return {
        resolved: false,
        reason: exactCollectorMatches.length
          ? "multiple_exact_collector_candidates"
          : "collector_not_found_in_catalog_manual_choice",
        candidates: source.slice(0, 30).map(x => ({
          id: x.card?.id,
          name: x.card?.name,
          set: x.card?.set?.name,
          setId: x.card?.set?.id,
    ...
    // No trustworthy full collector fraction:
    // never silently promote a visual guess to a confirmed database card.
    // Return ranked candidates so the Price Checker can ask the user to choose.
    return {
      resolved: false,
      reason: "collector_unverified_manual_choice_required",
      candidates: ranked.slice(0, 30).map(x => ({
        id: x.card?.id,
        name: x.card?.name,
        set: x.card?.set?.name,
        setId: x.card?.set?.id,
