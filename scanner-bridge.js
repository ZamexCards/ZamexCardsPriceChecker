/*
 ...
 The scanner runs on Vercel; the Price Checker runs on GitHub Pages.
 Modern mobile browsers partition third-party iframe localStorage, so
 scanned cards are transferred explicitly when the user opens Price Checker.
*/
(function(){
  const KEY="zc_favs";
  ...
  function importFromUrl(){
    ...
    if(Array.isArray(items)&&items.length){
      ...
      try{localStorage.setItem("zc_scanner_import_done",String(Date.now()))}catch(e){}
    }

    // Remove transfer payload from visible URL after import.
